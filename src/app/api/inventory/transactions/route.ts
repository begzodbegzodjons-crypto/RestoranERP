  ...
  try {
    ...
    const { rows } = await query(
      `SELECT it.*, i.name AS ingredient_name, u.name AS user_name
       FROM inventory_transactions it LEFT JOIN inventory i ON i.id = it.inventory_id LEFT JOIN users u ON u.id = it.user_id
       WHERE it.restaurant_id = ? ORDER BY it.created_at DESC LIMIT 100`,
