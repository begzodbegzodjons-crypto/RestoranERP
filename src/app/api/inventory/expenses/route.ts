...
export async function GET(req: NextRequest) {
  try {
    ...
    const { rows } = await query(
      `SELECT e.*, u.name AS created_by_name FROM expenses e LEFT JOIN users u ON u.id = e.paid_by
       WHERE e.restaurant_id = ? AND e.deleted_at IS NULL ORDER BY e.expense_date DESC, e.created_at DESC LIMIT 100`,
